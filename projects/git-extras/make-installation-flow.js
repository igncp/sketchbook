...
diagrams.layer(nl('git-extras make installation flow', [
    nl('sudo make install # without options and without environment variables', [
        ...
        nlc('For each COMMANDS_USED_WITH_GIT_REPO does:', 'nr', [
            nlc('Gets the first line of the file and the three helpers (in order: reset-env, git-extra-utility, is-git-repo) content and stores it in the temporal file'),
            nlc('Gets the script except the first two lines and append it to the temp file'),
            nl('Copies the file to the bins dir with its name')
        ]),
        nlc('For each COMMANDS_USED_WITHOUT_GIT_REPO does:', [
            nl('The same as in the loop before but without the is-git-repo helper')
        ]),
        nlc("If the man documments are not generated, it shows a warning, otherwise it copies them"),
